...
const UIStrings = {
  /** Title of an accesibility audit that evaluates if a table used for layout contains any data elements such as table headers as it can be confusing for screen readers. This descriptive title is shown to users when no user action is required. */
  title: 'Presentational `<table>` elements avoid using `<th>`, `<caption>` or the ' +
      '`[summary]` attribute.',
  /** Title of an accesibility audit that evaluates if a table used for layout contains any data elements such as table headers as it can be confusing for screen readers. This imperative title is shown to users when there is a failure that needs to be addressed. */
